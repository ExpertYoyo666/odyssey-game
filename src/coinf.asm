...
proc draw_coin3
  ...
  mov bx, offset coin_array3  ; the offset of the array
  mov al,[byte ptr bx+si]
  mov bh,0h
  mov cx,[bp-4h]  ; x
  mov dx,[bp-2h]  ; y
  ...
  sub ax,[bp+4h]
  cmp ax,[character_width]
  jge draw_char_inc_y_c3
  jmp draw_char_next_iter_c3
  draw_char_inc_y_c3:
  inc [word ptr bp-2h]
  mov ax,[bp+4h]
  mov [bp-4h],ax
  draw_char_next_iter_c3:
  inc si
  cmp si,[bp-6h]
  jl draw_char_loop_c3

  end_draw_character_c3:
  add sp, 6h
  pop bp
  ret
